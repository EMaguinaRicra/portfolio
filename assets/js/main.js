...
let WorkModal = function(workmodalClick){
  WorkViews[workmodalClick].classList.add('active-workmodal')
}
...
WorkClose.forEach((mc) => {
    mc.addEventListener('click', () =>{
      WorkViews.forEach((mv) => {
            mv.classList.remove('active-workmodal')
        })
    })
})
...
const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_vgxkyf2','template_u2hbt26','#contact-form','hnK4iTb9HGGHMysSY')
    .then(() =>{
        // Show sent message
